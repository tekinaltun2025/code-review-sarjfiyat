    ...
    <div className="container mx-auto px-4 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {campaignsData.map((campaign) => {
          // Get provider data safely (might be undefined during initial load)
          const provider = getProviderById(campaign.providerId);
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    {provider && provider.logo ? (
                      <img 
                        src={provider.logo} 
                        alt={provider ? provider.name : campaign.providerId} 
                        className="h-8 w-8 object-contain"
                      />
                    ) : (
                      <div className="h-8 w-8 bg-gray-200 rounded-full flex items-center justify-center">
                        {campaign.providerId.charAt(0).toUpperCase()}
                      </div>
                    )}
                    <span className="text-sm text-gray-600">
                      {provider ? provider.name : campaign.providerId}
                    </span>
                  </div>
                  {campaign.isNew && (
